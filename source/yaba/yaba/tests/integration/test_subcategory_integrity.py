...
from categories.views import SubCategoryView
...
class SubCategoryIntegrityTests(APITestCase):
    def setUp(self):
        ...
        self.sub_cat_view = SubCategoryView()

    def test_deleting_subcategory_reverts_transactions(self):
        request = self.factory.delete(f"/subcategories/{self.subcategory.pk}")
        request.user = self.user
        self.tr_view.setup(request)

        response = self.sub_cat_view.destroy(request, self.subcategory.pk)
