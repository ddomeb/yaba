...
from yaba.tests.test_utils import add_test_account, add_test_category, add_test_subcategory, add_test_transaction
...
class TransactionViewUnitTests(APITestCase):
    def setUp(self):
        ...
        self.transaction = add_test_transaction(self.user, subcategory_pk=self.subcategory.pk,
    ...
    def test_income_and_expense_transactions_listing(self):
        ...
        if query:
            self.assertEqual(
                response.data[0]["amount"], 1000 if query == 'in' else -1000, "Test transaction listed"
            )
        else:
            self.assertCountEqual(
                [tr["amount"] for tr in response.data], [1000, -1000]
            )
    ...
    def test_list_transactions_by_date(self):
        ...
        date_list_test_cases = [
            ...
            },
            {
                ...
                "dateto": None,
                "expected_count": 1,
                "expected_notes": [self.transaction.note, ],
            ...
            },
            {
                ...
                "expected_count": 1,
                "expected_notes": [new_transaction.note, ],
